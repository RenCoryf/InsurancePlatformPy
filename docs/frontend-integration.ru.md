...
| CRUD саппорт-агентов         | Python REST   | `/api/v1/admin/support-agents/` (Basic Auth)     |
...
Везде в `/api/v1/*` (кроме `/api/v1/admin/*`) — `Authorization: Bearer <JWT>`.
...
| Endpoint                                | Юзер | Саппорт | Внешний (Go) | Админ (Basic) |
|-----------------------------------------|------|---------|--------------|---------------|
...
В прод-окружении значения должны быть переопределены, иначе сервер не
стартует (есть startup-guard).
